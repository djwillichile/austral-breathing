...
La representatividad climática real (`client/public/data/representativeness_grid.json`) requiere los rasters WorldClim 2.1 desde `geodata.ucdavis.edu`. En este entorno ese host está bloqueado por egress (HTTP 403 `host_not_allowed`), por lo que el grid real queda pendiente. Para generarlo: habilitar egress a `geodata.ucdavis.edu` o precargar `wc2.1_*_bio_*.tif` en `data/climate/` y correr `python scripts_representativeness_analysis.py --region south-america --stations registry` (sin `--demo`).
