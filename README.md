...
- `scripts_build_outreach_plan.py` is the **Phase-2 data-request planner**: it
  selects the restricted-access CO₂ towers (25 tier-2 + 7 tier-3), parses each
  `institution_pi` into a best-effort institution / PI, derives a contact route
  and priority (P1 tier-2 with PI · P2 tier-2 without PI · P3 tier-3), and emits
  three artifacts — a per-tower contact sheet
  (`outputs/tables/phase2_outreach_contacts.csv`), a follow-up tracker
  (`outputs/tables/phase2_outreach_tracker.csv`, seeded once and preserved across
  regenerations), and a bilingual (ES/EN) plan with request-email templates
  (`research/phase2_outreach_plan.md`). Pure-stdlib and offline; the contact sheet
  and plan are regenerated by the `regenerate-registry-data` workflow, while the
  tracker keeps manual outreach progress.
